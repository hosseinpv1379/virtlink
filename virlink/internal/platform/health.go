...
func applyHealthPort(c *config.Config) {
...
func (h *HealthMgr) buildHealthJSON(tun core.Tunnel, bm *BenchMgr, probePort, httpPort int, selected string) healthJSON {
	hs := h.Handshake()
	httpStatus := "ok"
	if hs == "dead" {
		httpStatus = "degraded"
	}
	overlayPlain := core.PlainIP(tun.OverlayIP())
	iface := tun.DevName()
...
func (h *HealthMgr) runHTTPServer(overlayIP string, httpPort, probePort int, tun core.Tunnel, bm *BenchMgr) {
	...
	mux.HandleFunc("/api/interfaces", func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			"overlay_ip": core.PlainIP(tun.OverlayIP()),
			"peer_ip":    tun.PeerIP(),
			"http_port":  httpPort,
			"probe_port": probePort,
			"bench_port": httpPort + 1,
			"interfaces": collectIfaceStats(devs, core.PlainIP(tun.OverlayIP())),
		})
	})
...
func (h *HealthMgr) Start(overlayIP, peerIP string, probePort, httpPort int, tun core.Tunnel) {
