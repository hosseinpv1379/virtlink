...
func openvpnMultuRuntimeDir(c *config.Config) string {
	return filepath.Join("/var/run/virlink", TunnelInstanceName(c))
}

func OpenvpnMultuWorkerConfPath(c *config.Config, i int) string {
...
func OpenvpnMultuMaterializeWorkers(c *config.Config) (string, error) {
...
func openvpnMultuPerfMode(c *config.Config) string {
	switch tuningMode(c) {
	case tuningResource:
...
func openvpnMultuTunMTU(c *config.Config) (tunMTU, mssfix int) {
	proto := c.Transport.Proto
	if proto == "" {
		proto = "udp"
	}
