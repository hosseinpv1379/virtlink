...
func InitLogger(cfg *config.LoggingCfg) {
	...
	initProfiler(&config.Config{Logging: *cfg})
}
...
func PrintBanner(tunnelType, mode, localIP, remoteIP, overlay, peer, iface string,
	...
	if isatty() {
		...
		row("panel", fmt.Sprintf("http://%s:%d/  (health /health  bench /bench)", core.PlainIP(overlay), httpPort))
...
// Legacy lowercase aliases for same-package use.
const (
	cReset  = CReset
	cBold   = CBold
	cGray   = CGray
