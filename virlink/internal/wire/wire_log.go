...
func initWireMonitor(cfg *config.Config, path wirePathKind) {
...
func (m *WireMonitor) noteRxDrop(v wirePeerVerdict, src, dst [4]byte) {
	...
	if n <= 8 || n%100 == 0 {
		expect, _ := m.expectPeerWire.Load().(string)
		wireLogWarn(fmt.Sprintf("[wire] RX drop %s  outer src=%s dst=%s  want src=%s",
...
func (m *WireMonitor) noteTxNoDst() {
	if m.on.Load() {
		wireLogWarn("[wire] TX skipped: no peer route yet (server waiting for first RX?)")
	}
}
...
func WireLogHeartbeat() {
	...
	if tx > 0 && rx == 0 && wireMon.lastRxAt.Load() == 0 {
		wireLogWarn("[wire] TX>0 but RX=0 — client sending but server not receiving (firewall, rp_filter, wrong remote_ip, or peer down)")
		...
		if age := time.Since(time.Unix(0, wireMon.lastRxAt.Load())); age > 30*time.Second && tx > wireMon.rxOK.Load() {
			wireLogWarn(fmt.Sprintf("[wire] no peer RX for %s while TX active — one-way path broken?", age.Round(time.Second)))
		}
	}
}
