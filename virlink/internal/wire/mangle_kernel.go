...
func KernelTunnelWireUp(cfg *config.Config) error {
...
func KernelTunnelWireDown(cfg *config.Config) {
	if !WireSpoofEnabled(cfg) || !isKernelTunnel(cfg.Tunnel.Type) {
		return
	}
	restoreKernelMangle()
...
func TcpTunnelWireUp(cfg *config.Config) error {
	...
	wireLogOK("wire relay enabled (TCP: stack real IP + nft input/output like GRE)")
	logWireRelayRoles(cfg)
	return nil
}

func TcpTunnelWireDown(cfg *config.Config) {
	if !WireSpoofEnabled(cfg) || (cfg.Tunnel.Type != "tcp" && cfg.Tunnel.Type != "tcpmux") {
		return
	}
	TcpWireKernelDown(cfg)
...
func ApplyKernelMangle(cfg *config.Config) error {
	kernelMangleMu.Lock()
...
`, nftMangleTable, nftSpoofPrio, local, peerWireSrc, remote,
	...
	wireLogOK("wire relay enabled (kernel nft mangle)")
	logWireRelayRoles(cfg)
	return nil
}

// ApplyTCPWireMangle — stateless TCP wire relay (IRAN client / KHAREJ server).
func ApplyTCPWireMangle(cfg *config.Config) error {
	kernelMangleMu.Lock()
	defer kernelMangleMu.Unlock()
...
func tcpWireMangleScript(cfg *config.Config) string {
...
func logWireRelayRoles(cfg *config.Config) {
	wireSrc := cfg.Mangle.SrcIP
	peerWire := cfg.Mangle.DstIP
	peerReal := cfg.RemoteIP
	if cfg.Mode == "client" {
	...
	wireLogDebug(fmt.Sprintf("[wire] relay MSS=%d  nft table ip %s", mangleMSS, nftMangleTable))
}
